...
Бэкенд приватного навыка Яндекс Алисы, который отвечает на любые вопросы через **OpenAI**: по умолчанию быстрым путём (Responses API), с возможностью переключиться на durable Agent Session.
...
Ключевая идея: **историю разговора хранит OpenAI**, а не этот бэкенд. Один разговор с Алисой — один разговор на стороне OpenAI (Conversation на быстром пути, Agent Session на медленном). Мы не собираем и не пересылаем последние N сообщений: контекст помнит OpenAI, а Postgres хранит только связку «пользователь Алисы → id разговора».
...
flowchart TD
    ...
    Provider -->|AI_PROVIDER=responses| Responses[Responses API<br/>1,4-2,7 с]
    Provider -->|AI_PROVIDER=agents| Agents[Agents API<br/>9,5-12,2 с]
    Responses --> Conversation[(Conversation<br/>контекст на стороне OpenAI)]
    Agents --> Session[(Durable Agent Session)]
    Conversation --> Saved[Сохранённый агент<br/>model · instructions · tools]
    Session --> Saved
```

Вторая идея: **мы можем перестать ждать, но ответ от этого не пропадает**. У Яндекс Диалогов жёсткий лимит — 4,5 секунды. Если модель не успела, мы прекращаем ожидание и отвечаем «нужно ещё немного времени»; генерация продолжается на стороне OpenAI и дописывается в разговор, а следующей фразой «ну что» пользователь забирает готовый ответ.
...
| `src/ai` | Единственная граница с OpenAI: интерфейс `AiConversationProvider`, быстрый `OpenAIResponsesService`, медленный `OpenAIAgentsService`, `FakeAgentsProvider` для локальной проверки |
...
- Аккаунт OpenAI (для `AI_PROVIDER=agents` нужен доступ к Agents API beta)
...
| `AI_PROVIDER` | `responses` (по умолчанию, быстрый путь) или `agents` (durable Agent Session) |
...
|---|---|---|
| `ALICE_LLM_SOFT_TIMEOUT_MS` | `3700` | Сколько ждём ответ, прежде чем уйти в отложенный режим. Лимит Диалогов — 4500 мс, запас нужен на сеть |
...
### Бюджет времени и два пути к ответу

Диалоги ждут ответ 4,5 секунды — и в этот лимит входит всё: сеть Яндекса, наша обработка и поход в OpenAI.

Замеры с боевого сервера (агент `gpt-6-luna`, reasoning low, web_search включён):

| Что меряли | Время |
|---|---|
| Round-trip до OpenAI | ~0,9 с |
| **Responses API — готовый ответ** | **1,9–3,2 с** |
| Agents API — создание turn'а | 7,2–8,5 с |
| Agents API — готовый ответ | 9,5–12,2 с |

Разница не в модели: у Agents API около восьми секунд уходит только на то, чтобы **начать** turn, и это не зависит ни от reasoning effort, ни от инструментов, ни от `service_tier` (проверены `default`, `priority`, `fast`). Платформа рассчитана на долгие агентные задачи, а не на реплику в голосовом диалоге — в трейсе этой очереди даже не видно, там показано только время работы агента.

Поэтому по умолчанию работает **Responses API** (`AI_PROVIDER=responses`): модель, инструкции и встроенные инструменты берутся из того же сохранённого агента, так что настраивается всё по-прежнему в Platform UI, а контекст хранит OpenAI — теперь через Conversations API. Большинство вопросов получают ответ сразу, длинные объяснения уходят в отложенный режим.

`AI_PROVIDER=agents` возвращает durable Agent Session — он нужен, когда важнее не скорость, а долгая работа с инструментами и субагентами.

Что сделано, чтобы укладываться в лимит:

- дедлайн отсчитывается от прихода запроса и передаётся в каждый вызов к OpenAI;
- ретраи SDK на горячем пути отключены: повтор после таймаута удваивал ожидание (замер: 5,6 с при бюджете 2,5 с);
- прерывание ожидания не теряет ответ — он дописывается в разговор на стороне OpenAI, и follow-up забирает его из items;
- тем же бюджетом ограничены быстрые пути (получение готового ответа, проверка состояния).

`ALICE_LLM_SOFT_TIMEOUT_MS` по умолчанию 3700 мс — полный ответ Алисе укладывается примерно в 3,9 с, оставляя запас на сеть.

Две вещи, которые сильнее всего влияют на скорость:

- **`reasoning` и `verbosity` берутся из сохранённого агента и передаются в запрос.** Если их не передать, работает умолчание модели: замер на одном и том же вопросе — 2,0 с против 4,3 с.
- **Веб-поиск добавляет около трёх секунд** и ускорить его нельзя (`search_context_size: low` даже медленнее). Когда поиск начался, навык говорит «Посмотрю в интернете» вместо общего «нужно ещё немного времени» — это видно по событиям `response.web_search_call.*`. Чтобы поиск включался реже, сузьте правило в инструкциях агента: «пользуйся поиском только для свежих данных — новости, погода, курсы; факты, советы и подборки отвечай сам».
...
- **Длинные ответы приходят вторым шагом** — если модель не уложилась в бюджет, навык просит переспросить «ну что». На `AI_PROVIDER=agents` так работает почти каждый вопрос: см. замеры в разделе [«Бюджет времени»](#бюджет-времени-и-два-пути-к-ответу).
