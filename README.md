...
Бэкенд приватного навыка Яндекс Алисы, который отвечает на любые вопросы через OpenAI или Claude — на выбор, переключаемый голосом.
...
```
Яндекс Станция → Алиса → Яндекс Диалоги → этот бэкенд → OpenAI / Claude
```
...
2. Инструкции ассистента лежат в [`prompts/voice-assistant.ru.md`](prompts/voice-assistant.ru.md). Правьте файл (нужен передеплой) или переопределите переменной `ANTHROPIC_SYSTEM_PROMPT`.
...
**Что на самом деле стоит токенов.** Замер на проде: один вопрос к Claude — около 2600 входных токенов, из которых примерно 2200 занимает *описание инструмента веб-поиска*, отправляемое при каждом запросе независимо от того, искал ли он что-нибудь. Сам разговор на этом фоне почти не виден. Если поиск не нужен, `ANTHROPIC_WEB_SEARCH=false` сокращает вход почти в семь раз.

Промпт отправляется с `cache_control`, но **кэш на нём сейчас не срабатывает**: у Anthropic есть минимальная длина кэшируемого префикса, а голосовой промпт (~370 токенов) до неё не дотягивает — во всех боевых запросах `cache_creation_input_tokens` равен нулю. Брейкпоинт оставлен как задел: он ничего не стоит и начнёт работать сам, если промпт вырастет.
...
2. Заполните название и активационную фразу. Её произносят не только при запуске: «Алиса, спроси у дяди робота <вопрос>»
   ...
   Ту же фразу впишите в `ALICE_ACTIVATION_NAMES`: **внутри уже открытой сессии Яндекс её не вырезает**, и без этой
   настройки «спроси у дяди робота, что приготовить» дойдёт до модели целиком — она ответит, что не знает никакого дядю робота.
...
| `PENDING_STATE_TTL_SECONDS` | `86400` | Сколько ждёт неотданный ответ. Сутки — чтобы «ну что» работало и после того, как колонка погасла |
...
| `ALICE_ACTIVATION_NAMES` | — | Как вы зовёте навык вслух, через запятую. Обращение вырезается из вопроса внутри сессии |
