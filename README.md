...
|---|---|---|
| `ALICE_LLM_SOFT_TIMEOUT_MS` | `3500` | Сколько ждём ответ, прежде чем уйти в отложенный режим. Лимит Диалогов — 4500 мс, запас нужен на сеть |
...
| **Responses API — готовый ответ** | **1,4–2,7 с** |
...
`ALICE_LLM_SOFT_TIMEOUT_MS` по умолчанию 3500 мс — при этом полный ответ Алисе укладывается в ~3,7 с, оставляя запас на сеть.
