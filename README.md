...
### Бюджет времени и почему ответы почти всегда отложенные

Диалоги ждут ответ 4,5 секунды — и в этот лимит входит всё: сеть Яндекса, наша обработка и поход в OpenAI.

Замеры с боевого сервера (сентябрь 2026):

| Что меряли | Время |
|---|---|
| Round-trip до OpenAI (`sessions.list`) | ~0,9 с |
| Открытие сессии (`sessions.create`) | 1,1–2,2 с |
| **Создание turn'а** (`agent.session.turn.created`) | **7,2–8,5 с** |
| **Готовый ответ** на «скажи привет» | **9,5–12,2 с** |

Главное здесь — предпоследняя строка. Agents API тратит около восьми секунд только на то, чтобы **начать** turn, и это не зависит ни от модели, ни от `reasoning effort`, ни от включённых инструментов, ни от `service_tier` (проверены `default`, `priority`, `fast`). Платформа рассчитана на долгие агентные задачи, а не на реплику в голосовом диалоге.

Практическое следствие: **отложенный ответ — это основной режим работы навыка, а не редкий случай.** Диалог почти всегда выглядит так:

```text
— Что приготовить из курицы?
— Мне нужно ещё немного времени. Спроси меня через несколько секунд: ну что?
— Ну что?
— Можно потушить курицу с капустой: обжарь кусочки с луком…
```

Если такой ритм не подходит, вариант один: перевести обычные вопросы на Responses API (обычный инференс отвечает за 1–3 с), оставив Agents API для запросов, которым действительно нужны инструменты и долгая работа. Это меняет способ хранения истории, поэтому решение сознательное, а не техническая деталь.

Что сделано в коде, чтобы уложиться в лимит там, где это возможно:

- создание сессии и ожидание ответа идут **одним** streamed-запросом (`sessions.create({stream: true})`), а не двумя;
- дедлайн отсчитывается от прихода запроса и покрывает сам вызов создания сессии — иначе бюджет превышается ровно на её длительность;
- turn, за которым мы перестали следить, опознаётся по времени начала запроса: без этого «последним turn'ом» сессии мог оказаться ответ на предыдущий вопрос;
- тем же бюджетом ограничены и быстрые пути (получение готового ответа, проверка состояния сессии) — однажды такой запрос завис на 48 секунд, и без ограничения навык просто замолчал бы.

`ALICE_LLM_SOFT_TIMEOUT_MS` по умолчанию 2500 мс: ждать дольше нет смысла — ответ всё равно не успеет, — а запас до 4,5 с нужен реальный, на сеть Яндекса и TLS.
...
- **Ответы почти всегда приходят вторым шагом** — Agents API тратит ~8 секунд на запуск turn'а, а лимит Диалогов 4,5 с. Подробности и замеры в разделе [«Бюджет времени»](#бюджет-времени-и-почему-ответы-почти-всегда-отложенные).
