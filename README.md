...
1. Поднимите Postgres и Redis (managed или compose).
2. Выкатите образ: `docker compose up -d --build`.
3. Прогоните миграции (отдельным шагом, не при старте приложения).
4. Поставьте перед приложением HTTPS-терминатор (nginx, Caddy, Traefik) — Диалогам нужен валидный сертификат.
5. Настройте мониторинг на `/ready` и алерт на долю отложенных ответов: если она растёт, стоит понизить reasoning effort у агента.
