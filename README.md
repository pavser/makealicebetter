...
Диалоги ждут ответ 4,5 секунды — и в этот лимит входит всё: сеть Яндекса, наша обработка и поход в OpenAI. Из замеров на реальном сервере: один round-trip до OpenAI — около секунды, открытие сессии — 1,1–2,2 с.
...
- дедлайн отсчитывается от прихода запроса и покрывает в том числе сам вызов создания сессии — иначе бюджет превышается ровно на её длительность;
- при запуске навыка сессия **прогревается** в фоне, пока пользователь слушает приветствие, поэтому первый же вопрос идёт в готовую сессию.
